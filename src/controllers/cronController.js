...
const processInvoiceController = require("./Invoice/processInvoiceController");
const processCreditNoteController = require("./CreditNote/processCreditNoteController");
const processCancelInvoiceController = require("./CancelInvoice/processCancelInvoiceController");
...
const processAllForUser = async (user) => {
  ...
  try {
    ...
    await processCreditNoteController(user);
    await new Promise((r) => setTimeout(r, 1000));

    await processCancelInvoiceController(user);
  } catch (err) {
...
module.exports = startCronInvoice;
